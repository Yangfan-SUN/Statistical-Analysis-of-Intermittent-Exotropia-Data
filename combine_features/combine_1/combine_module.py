...
def combine_features(ex_dir, con_dir, res_dir, name, select, col, df1, df2):
    # load data
...
def generate_prob(col, select, 
    ...
    for res in os.listdir(results_dir):  
        if res.endswith('.csv'):  # csv  
            ...
            combine_features(ex_dir, con_dir, res_dir, name, select, col, df1, df2)
